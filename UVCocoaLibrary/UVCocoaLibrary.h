//
//  UVCocoaLibrary.h
//  UVCocoaLibrary
//
//  Created by chenjiaxin on 16/4/25.
//  Copyright © 2016年 hikvision. All rights reserved.
//

#import <UIKit/UIKit.h>

//! Project version number for UVCocoaLibrary.
FOUNDATION_EXPORT double UVCocoaLibraryVersionNumber;

//! Project version string for UVCocoaLibrary.
FOUNDATION_EXPORT const unsigned char UVCocoaLibraryVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <UVCocoaLibrary/PublicHeader.h>
#pragma mark - FMDatabase
//#import "FMDatabase.h"
//#import "FMDatabaseQueue.h"


#pragma mark - iToast
#import "UIView+Toast.h"

#pragma mark - MBProgressHUD
#import "MBProgressHUD.h"

//#pragma mark - XMLReader
//#import "XMLReader.h"

#pragma mark - Database
#import "UVIdEntity.h"
#import "UVTables.h"
#import "UVFMDatabase.h"

#pragma mark - Include
#import "UVConest.h"

#pragma mark - Media
#import "UVAudio.h"

#pragma mark - Model
#import "UVError.h"

#pragma mark - Request
#import "UVHttpClient.h"
#import "UVRequest.h"
#import "UVURLConnectionDataDelegate.h"
#import "UVSoap.h"

#pragma mark - Utils
#import "UVUtils.h"
#import "UVDevice.h"
#import "UVLogger.h"
#import "UVCache.h"
#import "UVValidate.h"
#import "UVFileManage.h"

#pragma mark - Widget
#import "UVTableView.h"
#import "UVModelView.h"
#import "UVActivityIndicatorView.h"
#import "UVMenuButton.h"
#import "UVScrollView.h"
#import "UVToolbarDatepickerView.h"
#import "UVCollectionView.h"

#pragma mark - Category
#import "NSObject+UVObjects.h"
#import "UIColor+UVUtils.h"
#import "UIImage+UVUtils.h"
#import "UITableView+UVUtils.h"
#import "UITextField+UVUtils.h"
#import "UIView+Shake.h"
#import "UIView+UVUtils.h"
#import "UIViewController+UVUtils.h"
#import "NSDate+UVUtils.h"
#import "NSString+UVUtils.h"
#import "UIScrollView+UVUtils.h"
#import "NSMutableArray+UVUtils.h"
#import "UVSafeData.h"

